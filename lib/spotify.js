...
export const getEpisode = async () => {
  ...
  const EPISODE_ENDPOINT = `https://api.spotify.com/v1/episodes/512ojhOuo1ktJprKbVcKyQ`;

  return fetch(EPISODE_ENDPOINT, {
    headers: {
      Authorization: `Bearer ${access_token}`,
    },
  });
...
export const searchForPodcasts = async (q) => {
  const { access_token } = await getAccessToken();

  const SEARCH_ENDPOINT = `https://api.spotify.com/v1/search?type=show&include_external=audio&q=${q}`;
