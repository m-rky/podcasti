...
export const getEpisodes = async (id) => {
  ...
  const res = await fetch(`${base}/episodes/byfeedid?id=${id}&max=1000`, options);
...
export const getTitle = async (id) => {
  const res = await fetch(`${base}/podcasts/byfeedid?id=${id}&max=1&pretty`, options);
  return await res.json();
};
export const getMultiple = async (id) => {
  const res = await fetch(`${base}/episodes/byfeedid?id=${id}&max=3&pretty`, options);
  return await res.json();
};
